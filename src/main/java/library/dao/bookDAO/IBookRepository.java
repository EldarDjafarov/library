...
public interface IBookRepository extends JpaRepository<Book, Long> {
    ...
    @Query("select  distinct u from Book u left join fetch u.genres genr " +
            " left join fetch u.authors aut where u.id=?1 or u.title=?2 or u.date=?3 or genr.name=?4 or aut.name=?5 or u.numberOfBooks=?6")
